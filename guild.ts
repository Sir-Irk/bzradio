...
export class user_guild {
	...
	playingEmbed: Discord.EmbedBuilder;
	progressMessage: Discord.Message;
	loopMode: boolean = false;
	lastSongPlayed: playlist_entry;

	textChannel: Discord.DMChannel;
	player;
...
export async function play_song(guild: user_guild, song: playlist_entry) {
	...
	while (attempts < fetchUrlMaxAttempts) {
		try {
			...
			guild.resource = createAudioResource(stream);
